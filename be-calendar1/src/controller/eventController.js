...
module.exports = (container) => {
  ...
  function formatDataUpdate (data, dataGoogle) {
    ...
    if ((data.attendees || []).length === 0) {
      ...
      const qq = data.attendees.map(va => {
        return { email: va }
      })
      qq.attendees = [...zz, ...qq]
    }
      ...
      if (event.checkEdit === 0) {
        ...
        if (event.accounts && event.accounts.length) { // co account
          ...
          if (!event.booking) { // gio moi booking
            ...
            const { ok, data: d } = await googleHelper.getEventById(data.refreshToken, calendarId)
            if (!ok) {
            ...
            const { ok: o, data: re } = await googleHelper.updateCalendar(data.refreshToken, calendarId, event)
            if (!o) {
              return res.status(httpCode.BAD_REQUEST).json({ msg: 'co loi trong qua trinh dong bo calendar' })
            }
